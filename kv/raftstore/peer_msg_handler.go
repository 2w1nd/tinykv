...
func (d *peerMsgHandler) applyRequest(entry *eraftpb.Entry, msg *raft_cmdpb.RaftCmdRequest, wb *engine_util.WriteBatch) *engine_util.WriteBatch {
	...
	d.handleProposal(entry, func(p *proposal) {
		...
		case raft_cmdpb.CmdType_Get:
			...
			value, err := engine_util.GetCF(d.peerStorage.Engines.Kv, req.Get.GetCf(), req.Get.GetKey())
			if err != nil {
				value = nil
			}
...
func (d *peerMsgHandler) HandleRaftReady() {
	...
	if d.RaftGroup.HasReady() {
		rd := d.RaftGroup.Ready() // 生成一个包含所有待处理信息的Ready结构
...
func (d *peerMsgHandler) proposeRequest(msg *raft_cmdpb.RaftCmdRequest, cb *message.Callback) {
	...
	if err != nil {
		log.Errorf("propose data error: %v", err)
		return
	}
}
